        ...
        public bool MainRoutine()
        {
            try
            {
                ...
                while (user_number <= task.num_users_init)
                {
                    ...
                    else //if there is no top job recommended for a user
                    {
                        writeTextAverages.WriteLine("USER: " + users_profile[user_number - 1].UserID);
                        writeTextAverages.WriteLine("RATING AVGS TOTAL 0.00\t");
                        writeTextAverages.WriteLine("PERCENTAGE AVGS TOTAL 0.00\n");
                        writeText.WriteLine(users_profile[user_number - 1].UserID + "\t 0");
                        users_calculated_raitings[user_number - 1] = 0;
                    }
